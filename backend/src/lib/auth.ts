...
import { CLIENT_ORIGIN, CLOUDFLARE_SECRET_KEY, JWT_ACCESS_TOKEN_SECRET, JWT_REFRESH_TOKEN_SECRET, NODE_ENV } from "../config/env.js";
dotenv.config()
...
export async function issueTokens(user:User,res:Response):Promise<AccessToken> {
  ...
  const deviceId = v4()
  //refresh token
  const refreshToken = jwt.sign({ userId: user.id, deviceId }, JWT_REFRESH_TOKEN_SECRET, {
    expiresIn: "7d",
  });
