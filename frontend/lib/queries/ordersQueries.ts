...
export async function makeOrder():Promise<> {
    const res = await fetch(`${apiBaseUrl}/orders`, { credentials: "include", method: "POST" })
    return await handleResponse(res)
}

//will probably be run when on cancel route
export async function cancelOrder(orderId: string): Promise<> {
    const res = await fetch(`${apiBaseUrl}/orders/${orderId}/cancel`, { credentials: "include", method: "POST" })
    return await handleResponse(res);
}